...
export async function createInvestment(userId, amount, provider, details) {
    try {
        // Fetch all plans and use the one with the lowest minimum amount
        const plansResult = await investmentsAPI.getPlans();
        let planId = null;

        if (plansResult.success && plansResult.data.plans.length > 0) {
            // Sort plans by minimum amount and use the lowest one
            const sortedPlans = [...plansResult.data.plans].sort((a, b) => a.minAmount - b.minAmount);
            planId = sortedPlans[0].id;
        }

        if (!planId) {
            return { success: false, error: 'Investment plans not available' };
        }
